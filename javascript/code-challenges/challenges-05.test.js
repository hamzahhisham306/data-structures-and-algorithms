...
------------------------------------------------------------------------------------------------ */
const toLastNames = people => people.map(acc=>`${acc.firstName} ${acc.lastName}`);
...
const addValues = (arr) => arr.reduce((acc, current) => {
    return acc + current;
  }, 0);
...
const addPurchases = (arr) => {
  let sumPur=arr.reduce((acc, current)=>{
...
const countNumberOfElements = (arr) =>arr.reduce((acc,current, index)=>{
  return index+1;
});
/* ------------------------------------------------------------------------------------------------
CHALLENGE 5
...
const returnNames = (arr) => {
  let justnames = arr.reduce((acc, current) => [...acc, current.name],[]);
...
const reversedString = (str) => str.split('').reduce((acc, current)=>{
  return `${current}${acc}`;
});
/* ------------------------------------------------------------------------------------------------
...
const countNumberOfChildren = (arr) => {
  let counter=0;
  let nums=arr.reduce((acc, current)=>{
        if(current.children){
          counter+=current.children.length;
        }
        return counter;
  },0)
  return nums;
...
const calculateAverage = (arr) => {
  // Solution code here...
  let avert=arr.reduce((acc,current)=> acc+current);
   return avert/arr.length;
};
...
const countPrimeNumbers = (arr) => {
  // Solution code here...
  let arrays=[];
  let counter=arr.reduce((acc,current)=>{
    return arrays.push(isPrime(acc), isPrime(current));
    });
  let countnumber=arrays.filter((num)=>{
    return num==true;
  })
