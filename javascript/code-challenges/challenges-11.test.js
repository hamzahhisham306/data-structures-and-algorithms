...
const count = (target, input) => {
  //   // Another Soultion
  // let count= input.map(num=>num.filter(num=>num===target).length).reduce((acc, current)=> acc+current);
  // return count;
  let count=0;
...
const totalSum =input=>input.map(num=>num.reduce((acc, current)=>acc+current)).reduce((acc, current)=>acc+current);
...
const divisibleByFiveTwoToThePower = input=>input.map(num=>num.filter(num=>typeof num==='number')).map(num=> num.filter(num=>num%5===0).map(num=> Math.pow(2,num)));
/* ------------------------------------------------------------------------------------------------
CHALLENGE 5
...
let findMaleAndFemale = data=> data.filter((str)=>str.gender==='female' || str.gender==='male').map(str=> str.name).join(' and ');
...
let findShortest = (data) => {
  ...
  let dataSorted=datafilter.sort((a,b )=>{
    return a.height-b.height;
  });
  return dataSorted[0].name;
};
