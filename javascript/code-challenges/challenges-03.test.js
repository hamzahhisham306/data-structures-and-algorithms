...
const containsAnd = arr => arr.filter(str=>str.match(/and/ig));
...
const notInFirstArray = (forbiddenValues, arr) => {
  let notin=arr.filter(num=>!forbiddenValues.includes(num));
  return notin;
}
...
const getBaseStatGreaterThan = (arr, minBaseStat) =>arr.filter(str=>str.baseStat>minBaseStat);
...
const getStatName = (arr, minBaseStat) => arr.filter(str=>str.baseStat>minBaseStat).map(str=>str.stat.name);
...
const evenOddNumericValues = arr =>arr.filter(num=>typeof num==='number').map(num=>num%2===0?'even':'odd');
