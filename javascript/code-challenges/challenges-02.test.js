...
const raisedToTheThird = (arr) => {
  // Solution code here...
  let newPow=[];
  arr.forEach(n=>newPow.push(Math.pow(n,3)));
  return newPow;
};
...
const addQuestion =arr=>arr.map(s=>s.concat('?'));
...
const extractAbilities =arr=> arr.abilities.map(str=>str.ability.name);
...
const extractStats = arr =>arr.stats.map(str=>{
  return {
    name:str.stat.name,
    total:str.effort+str.baseStat
  }
});
