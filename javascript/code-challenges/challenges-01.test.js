...
const addOne = (arr) => {
  // Solution code here...
  let newArray=[];
  arr.forEach(n=>newArray.push(n+1));
  return newArray;
};
...
const addExclamation = (arr) => {
  // Solution code here...
  let newStr=[];
  arr.forEach(str=>newStr.push(str.concat('!')));
  return newStr;
};
...
const allUpperCase = (arr) => {
  // Solution code here...
  let newUpper=[];
  arr.forEach(str=>newUpper.push(str.toUpperCase()));
  return newUpper;
};
...
const greeting = (word) => {
  // Solution code here...
  let newGreeting=[];
  word.forEach(arr=>newGreeting.push(arr.toUpperCase().concat('!')));
  return newGreeting;
};
...
const addNumbers = (num, arr, times, callback) => {
  const addNumber = Array.apply(null, Array(times));
  addNumber.forEach(()=> {
    callback(arr, num);
  });
  return arr;
...
const createList = (availableItems) => {
  // Solution code here...
  let newList=[];
  availableItems.forEach(fru=>fru.available===true?newList.push(fru.name):'');
  return newList;
};
