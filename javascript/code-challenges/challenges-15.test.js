...
const screenForNames = (arr) => {
  // Solution code here...
  const reguller = /^(Mr\.|Mrs\.|Ms\.|Dr\.)\s[a-zA-Z]+$/;
  let arrayreg=arr.filter((n)=>{
    return reguller.test(n);
  })
  return arrayreg;
}
...
const sortBy = (property, arr) => {
  // Solution code here...
  return arr.sort((acc, curernt) => {
    if (acc[property]<curernt[property]) {
      return -1;
    } else if (acc[property]>curernt[property]) {
      return 1;
    } else {
      return 0;
    }
...
Instead of trying to write crazy for loops to automate checking the rows, columns and diagonals consider writing one helper function that accepts three coordinate pairs and checks the values of the array at those locations. For instance helpCheck(row1, col1, r2, col2, row3, col3).
...
const detectTicTacToeWin = (board) => {
  // Solution code here...
  function help(r1,c1,r2,c2,r3,c3) {
    return board[r1][c1] === board[r2][c2] && board[r2][c2] === board[r3][c3] && board[r1][c1] !== '';
  }
  for (let i = 0; i < board.length; i++) {
    if (help(i, 0, i, 1, i, 2)) return true;
  }
  for (let j = 0; j < board.length; j++) {
    if (help(0, j, 1, j, 2, j)) return true;
  }
  if (help(0, 0, 1, 1, 2, 2)) return true;
  if (help(0, 2, 1, 1, 2, 0)) return true;
  return false;
};
