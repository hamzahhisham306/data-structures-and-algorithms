...
// Solution code here...
let longestStr = arr.reduce((acc, current) => {
...
const standardizePhoneNumbers = (arr) => {
  ...
  for (let i=0;i<arr.length; i++) {
    phonenubmer.push(arr[i].replace(/[()-\s]/g,""));
  }
  return phonenubmer;
};
...
const onlyOddChars = (str) => {
  // Solution code here...
  let oddchar=str.split("").filter((str, i)=>{
    return i%2!==0;
  }).join('');
  return oddchar;
};
